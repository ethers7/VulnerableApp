...
public class EncryptionUtils {
    ...
    public static String decrypt(String ciphertext, SecretKey key) throws EncryptionException {
        try {
            byte[] ivAndCiphertext = java.util.Base64.getDecoder().decode(ciphertext);

            if (ivAndCiphertext.length < GCM_IV_LENGTH_BYTES) {
                throw new EncryptionException("Ciphertext is too short to contain a valid IV");
            }

            byte[] iv = new byte[GCM_IV_LENGTH_BYTES];
            System.arraycopy(ivAndCiphertext, 0, iv, 0, GCM_IV_LENGTH_BYTES);

            byte[] encrypted = new byte[ivAndCiphertext.length - GCM_IV_LENGTH_BYTES];
            System.arraycopy(
                    ivAndCiphertext, GCM_IV_LENGTH_BYTES, encrypted, 0, encrypted.length);
            ...
            throw new EncryptionException(
                    "AES decryption failed — wrong key or corrupted ciphertext", e);
        }
    }
}
